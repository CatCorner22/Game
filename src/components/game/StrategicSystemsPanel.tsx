...
function Gauge({
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-3 font-mono text-[10px] tracking-wider uppercase">
  ...
  return (
    <section className="mt-5 rounded-lg bg-elevated p-3 shadow-[var(--shadow-border)]" aria-labelledby="strategic-systems-title">
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={cn(
            "rounded-full px-2 py-1 font-mono text-[9px] tracking-wider uppercase",
      ...
      <div className="mt-3 rounded-md bg-bg/70 p-3">
        <div className="flex items-center justify-between gap-3">
          <span className="font-mono text-[10px] tracking-wider text-muted uppercase">Recommendation</span>
      ...
      {ai.mode === "ensemble" || ai.mode === "skynet" ? (
        <details className="mt-3 rounded-md bg-bg/55 p-3">
          <summary className="cursor-pointer font-mono text-[10px] tracking-wider text-muted uppercase">
            Compare model votes
          </summary>
          <ul className="mt-3 space-y-2">
            {ai.modelVotes.map((vote) => (
              <li key={vote.model} className="border-l border-border pl-3">
                <div className="flex items-center justify-between gap-3">
                  <span className="font-mono text-[10px] text-fg">{vote.model}</span>
                  <span className="font-mono text-[10px] text-accent uppercase">
                    {vote.action} · {Math.round(vote.confidence)}
                  </span>
                </div>
                <p className="mt-1 text-[11px] leading-relaxed text-subtle">{vote.note}</p>
              </li>
            ))}
          </ul>
        </details>

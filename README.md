...
- The fictional AI systems — ORACLE, CHORUS, SKYNET, DEADHAND — run locally as deterministic game logic. They do not connect to an external LLM, military system, or live data source, and they never will: a language model must never be the thing recommending a nuclear strike.
- Human advisors in the warning conference may have their *dialogue* generated by an external language model, if you configure one. That is prose only. Every advisor's recommendation, every option you are offered, and every consequence is computed locally by the same deterministic simulation that runs when no model is configured — which is the default, and what continuous integration and offline play use.
...
| `ADVISOR_API_KEY` | unset | Enables generated advisor dialogue. Server-side only — never `VITE_`-prefixed, never in `.grok/app-env.json` (that file is committed and only carries `VITE_` build flags) |
| `ADVISOR_BASE_URL` | `https://router.huggingface.co/v1` | Any OpenAI-compatible endpoint. Must be absolute `http(s)` |
| `ADVISOR_MODEL` | `meta-llama/Llama-3.3-70B-Instruct` | Model id passed to the provider |

#### Generated advisor dialogue

With no key set — the default — the warning conference runs entirely on
hand-written dialogue. That is the path CI takes and the path offline play
takes, and it is a complete experience rather than a degraded one.

With a key set, `POST /api/advisor` re-voices one advisor line at a time
through an OpenAI-compatible provider. The key is read inside the route
handler and never reaches the browser. What crosses the boundary is a short
whitelisted summary — confidence, boost count, azimuth, minutes, alert state —
never the raw world. Anything the player types is passed as user content, never
as a system instruction.

The model can only change *wording*. Every recommendation, every option, and
every consequence is computed locally either way: a provider outage, a timeout,
a rate limit, or a hostile response all resolve to the same hand-written line.
Note that `server/routes/**` is only mounted for `build`/`preview`, not
`vite dev`, so verify the route with `npm run preview`.
